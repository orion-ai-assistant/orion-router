...
- **localStorage**: Kullanıcının seçtiği locale'i `"orion_locale"` anahtarı altında kalıcı olarak saklayan tarayıcı depolama alanı.
...
1. WHEN kullanıcı dashboard'u ilk kez açtığında, THE I18n_System SHALL `localStorage`'da `"orion_locale"` anahtarını kontrol etmeli ve kayıtlı bir değer varsa o Locale'e ait Locale_File'ı yüklemelidir.
2. WHEN `localStorage`'da kayıtlı Locale bulunmadığında, THE I18n_System SHALL `navigator.language` değerini okuyarak Browser_Locale'i belirlemeli ve eğer bu değer Supported_Locales listesinde yer alıyorsa ilgili Locale_File'ı yüklemelidir.
...
4. WHEN Browser_Locale değeri tam eşleşme olmaksızın yalnızca dil kodu içerdiğinde (örn. `"tr-TR"`), THE I18n_System SHALL dil kodunu (`"tr"`) ayıklayarak Supported_Locales içinde arama yapmalı ve eşleşen Locale_File'ı yüklemelidir.
5. WHEN Locale_File yüklendiğinde, THE I18n_System SHALL dosyanın geçerli bir JSON nesnesi içerdiğini doğrulamalı ve geçerli ise çeviri deposuna aktarmalıdır.
...
5. WHEN ikinci argüman sağlanmadığında veya bir yer tutucu için değer eksik olduğunda, THE t() Fonksiyonu SHALL yer tutucuyu olduğu gibi bırakmalı, hata fırlatmamalıdır.
...
1. THE AppContext SHALL `locale` (aktif Locale kodu) ve `setLocale` (Locale kodu kabul eden ve locale değişimini tetikleyen fonksiyon) değerlerini context değeri olarak sağlamalıdır.
2. WHEN `setLocale` çağrıldığında, THE AppContext SHALL yeni Locale değerini `localStorage`'da `"orion_locale"` anahtarı altında saklamalıdır.
...
5. THE AppContext SHALL `t()` fonksiyonunu da context değeri olarak sağlamalı; böylece tüm alt bileşenler `useApp()` hook'u aracılığıyla çeviri işlevine erişebilmelidir.
6. WHILE uygulama çalışırken, THE AppContext SHALL Supported_Locales listesinin tamamını context değeri olarak sunmalıdır.

---

### Requirement 4: Locale Kalıcılığı

**User Story:** Bir dashboard kullanıcısı olarak, dil tercihimin tarayıcıyı kapattıktan sonra da hatırlanmasını istiyorum; her oturum açtığımda dilin sıfırlanması beni yoruyor.

#### Kabul Kriterleri

1. WHEN kullanıcı `setLocale` aracılığıyla bir Locale seçtiğinde, THE I18n_System SHALL seçilen Locale kodunu `localStorage`'da `"orion_locale"` anahtarına yazmalıdır.
2. WHEN dashboard sonraki oturumda yüklendiğinde, THE I18n_System SHALL `localStorage`'dan `"orion_locale"` değerini okumalı ve bu değer Supported_Locales içinde geçerliyse o Locale'i etkin olarak ayarlamalıdır.
...
2. THE Language_Panel SHALL aktif Locale'i ve Supported_Locales içindeki tüm 43 dili seçilebilir şekilde listelemelidir.
3. THE Language_Panel SHALL her dilin kendi dilindeki yerel adını (örn. "Türkçe", "Deutsch", "日本語") ve varsa İngilizce etiketini birlikte göstermelidir.
...
6. THE Language_Panel SHALL aktif dili görsel olarak vurgulamalıdır (örn. aktif seçenek üzerinde bir işaret veya renk değişikliği).
...
1. THE I18n_System SHALL `DashboardLayout.tsx` içindeki `TABS` dizisinde tanımlı tüm sekme etiketlerini (`"Overview"`, `"Virtual Keys"`, `"Logs"`, `"Provider Keys"`, `"Models"`, `"Groups"`, `"Playground"`, `"Model Info"`, `"Settings"`) Translation_Key karşılıklarıyla çevirmelidir.
...
1. THE I18n_System SHALL aşağıdaki sayfaların tüm statik metin içeriklerini çevirmelidir: Overview (`/`), Virtual Keys (`/keys`), Logs (`/logs`), Provider Keys (`/key-pool`), Models (`/models`), Groups (`/groups`), Playground (`/playground`), Model Info (`/model-info`), Settings (`/settings`).
...
3. THE I18n_System SHALL `AppContext.tsx` içindeki tüm dialog ve modal başlıklarını, açıklama metinlerini ve buton etiketlerini çevirmelidir (login dialog, confirm dialog dahil).
...
3. THE I18n_System SHALL Fallback_Locale dosyasını (`en.json`) uygulama başlangıcında bir kez yüklemeli ve tüm oturum boyunca bellekte tutmalıdır.
4. IF aktif Locale, Fallback_Locale ile aynıysa (`en`), THEN THE I18n_System SHALL yalnızca tek bir locale dosyası yüklemeli, ikinci bir yükleme gerçekleştirmemelidir.
...
4. WHEN Locale değiştiğinde, THE I18n_System SHALL `dir` ve `lang` özniteliklerini sayfa yenilemesi gerektirmeksizin güncellenmiş değerlere çevirmelidir.
...
1. THE I18n_System SHALL tüm Translation_Key'leri `{namespace}.{key}` biçiminde organize etmeli; namespace değerleri şunları kapsamalıdır: `nav` (sidebar), `common` (paylaşılan buton/etiketler), `auth` (giriş/kimlik doğrulama), `settings` (Settings sayfası), `keys` (Virtual Keys sayfası), `logs` (Logs sayfası), `keyPool` (Provider Keys sayfası), `models` (Models sayfası), `groups` (Groups sayfası), `playground` (Playground sayfası), `modelInfo` (Model Info sayfası), `overview` (Overview sayfası).
2. THE Locale_File SHALL geçerli bir JSON nesnesi olmalı ve en az Fallback_Locale (`en`) Locale_File'ında tanımlı tüm Translation_Key'leri içermelidir.
3. THE I18n_System SHALL `en.json`'ı tüm Locale_File'lar için referans şema olarak kabul etmeli; diğer dillerde eksik anahtarlar Fallback Davranışı (Gereksinim 8) kuralları çerçevesinde karşılanmalıdır.
...
1. THE I18n_System SHALL `dashboard/lib/i18n.ts` dosyasında tanımlanmalı ve şu dışa aktarımları içermelidir: `detectLocale()`, `loadLocale(lang: string)`, `createTranslator(translations: Record<string, string>, fallback: Record<string, string>)`.
2. THE `detectLocale()` fonksiyonu SHALL önce `localStorage["orion_locale"]`'i, ardından `navigator.language`'i kontrol etmeli, Supported_Locales ile eşleştirmeli ve geçerli bir Locale kodu veya `"en"` döndürmelidir.
...
4. THE `createTranslator(translations, fallback)` fonksiyonu SHALL aktif ve fallback çeviri nesnelerini kapatan (closure) ve `t(key, vars?)` imzasına sahip bir fonksiyon döndürmelidir.
